...
Task.prototype.loop = function (callback) {
  ...
  async.until(function () { return self.stopCriteria() }
            , function (cb) { self.iteration(function () { process.nextTick(cb) }) }
            , function (err) {
              if (err!=null) { self.emit('error', err) }
              else { 
                self.emit('loop end')
...
Task.prototype.reproduction = function (callback) {
  ...
  async.series([
      ...
      , function (callback) {
        ...
        var maxScore = Math.abs(max.score)
        if (self.minimize) {
          async.forEach(self.parents
            , function (item, cb) {
              item.score*=-1
      ...
      //       }
      //       else {
      //         self.emit('normalize end', self.parents)
      //         var i,s =0 
      //         for (i=0;i<self.popSize;i++) {
      //           s+=self.parents[i].score
      //         }
      //         console.log('sum of normalized',s)
      //         callback()
      //       }
      //     }
      //   )
      ...
      , function (callback) {
        ...
        async.until(
            ...
            , function (cb) {
              ...
              // console.log(position)
              // console.log(secondParentPosition)
              // console.log(self.parents.length)
