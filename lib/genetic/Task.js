    ...
    async.forEach(self.population, function (item, cb) {
        ...
        if (err != null) {
            ...
            if (self.globalStatistics == undefined) {
                self.globalStatistics = JSON.parse(JSON.stringify(self.statistics));
            } else {
                if (self.globalStatistics.maxScore < max.score) {
                    self.globalStatistics.maxScore = max.score;
                    self.globalStatistics.max = JSON.parse(JSON.stringify(max));
                }
                if (self.globalStatistics.minScore > min.score) {
                    self.globalStatistics.minScore = min.score;
                    self.globalStatistics.min = JSON.parse(JSON.stringify(min));
                }
            }
