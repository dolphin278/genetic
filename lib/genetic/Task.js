...
Task.prototype.calcFitness = function (callback) {
    ...
    async.forEach(self.population, function (item, cb) {
...
Task.prototype.parentSelection = function (callback) {
    ...
    self.parents = [];
    async.forEach(self.population, function (item, callback) {
...
Task.prototype.reproduction = function (callback) {
    ...
    async.series([function (callback) {
        ...
        async.forEach(self.parents, function (item, cb) {
            ...
            if (err != null) {
                ...
                self.statistics = {
                    minScore: min.score,
                    maxScore: max.score,
                    min: JSON.parse(JSON.stringify(min)), // TODO optimize
                    max: JSON.parse(JSON.stringify(max)), // TODO optimize
        ...
        async.until(function () {
            ...
            var child, parent, secondParent, secondParentPosition;
            while (point > level) {
                position++;
                level += self.parents[position].score;
            }
            child = JSON.parse(JSON.stringify(self.parents[position])); // TODO optimize // TODO Look why
